...
namespace Bison.Framework.Extensions
{
    ...
    public static class SpriteBatchExtensions
    {
        ...
        {
            return DrawInt32(spriteBatch, spriteFont, value, position, color, 1);
        }
        ...
        {
            return DrawInt64(spriteBatch, spriteFont, value, position, color, 1);
        }
        ...
        /// <param name="position">The screen position specifying where to draw the value.</param>  
        /// <param name="color">The color of the text drawn.</param>
        /// <param name="minDigits">The minimal length of the number.</param>  
        /// <returns>The next position on the line to draw text. This value uses position.Y and position.X plus the equivalent of calling spriteFont.MeasureString on value.ToString(CultureInfo.InvariantCulture).</returns>  
        public static Vector2 DrawInt32(this SpriteBatch spriteBatch, SpriteFont spriteFont, int value, Vector2 position, Color color, int minDigits)
        {
            if (spriteBatch == null) throw new ArgumentNullException("spriteBatch");
            if (spriteFont == null) throw new ArgumentNullException("spriteFont");
            if (minDigits < 1) throw new ArgumentException("digits must be positive.");
            ...
            else
            {
                ...
                // render leading zeros
                for (int i = minDigits - index - 1; i >= 0; --i)
                {
        ...
        /// <param name="minDigits">The minimal length of the number.</param>  
        /// <returns>The next position on the line to draw text. This value uses position.Y and position.X plus the equivalent of calling spriteFont.MeasureString on value.ToString(CultureInfo.InvariantCulture).</returns>  
        public static Vector2 DrawInt64(this SpriteBatch spriteBatch, SpriteFont spriteFont, long value, Vector2 position, Color color, int minDigits)
        {
            if (spriteBatch == null) throw new ArgumentNullException("spriteBatch");
            if (spriteFont == null) throw new ArgumentNullException("spriteFont");
            if (minDigits < 1) throw new ArgumentException("digits must be positive.");
